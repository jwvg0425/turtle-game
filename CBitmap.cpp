...
void CBitmap::Draw(HDC hdc,int x,int y)
{
	Draw(hdc,x,y,width,height);
}

void CBitmap::Draw(HDC hdc,int x,int y,int w,int h,int sx/*=0*/,int sy/*=0*/)
{
	HDC MemDC;
	HBITMAP OldBitmap;
	assert(hBit != NULL);
...
void CBitmap::Stretch(HDC hdc,int x,int y,int w,int h,int sx,int sy,
	int sw/*=-1*/,int sh/*=-1*/)
{
	HDC MemDC;
	HBITMAP OldBitmap;
...
void CBitmap::Draw(HDC hdc,int x,int y,COLORREF Mask)
{
	BITMAP bm;
	COLORREF cColor;
